document.addEventListener('DOMContentLoaded', () => {
  ...
  // Delegação de evento global para abrir o carrinho ao clicar no botão do header
  ...
  // NOVO: Delegação global para capturar os cliques nos botões "Selecionar" / "Comprar" 
  // (Funciona tanto na Home quanto no Catálogo gerado por JSTL)
  document.addEventListener('click', (event) => {
    const btn = event.target.closest('.agnello-btn-comprar');
    if (!btn) return;

    // Se o botão já tiver um onclick inline, deixa o navegador executá-lo
    ...
    const vinhoObj = {
      ...
      origem: btn.getAttribute('data-origem'),
      imagem: btn.getAttribute('data-imagem')
    };

    if (vinhoObj.nome && vinhoObj.preco) {
      window.adicionarAoCarrinho(vinhoObj);
    }
  });
  ...
  function renderizarConteudoCarrinho() {
    ...
    if (carrinho.length === 0) {
      ...
      let totalGeral = carrinho.reduce((acc, item) => {
        let precoNum = parseFloat(String(item.preco).replace('R$', '').replace('.', '').replace(',', '.').trim()) || 0;
